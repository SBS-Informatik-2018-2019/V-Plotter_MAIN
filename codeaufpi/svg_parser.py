    ...
    while len(path_elements) != 0:
        ...
        if(path_elements[0] == "M" or path_elements[0] == "m"):
           ...
           # print(path_elements[0: index])
            ...
            # der Startpunkt wird f�r das Z-Element gesetzt
        ...
        # eine Zeichnung wird zum Startpunkt zur�ckgef�hrt
        elif(path_elements[0] == "Z" or path_elements[0] == "z"):
            #print("Zur�cklauf - Zeichnen zur�ck zum Startpunkt")
            # laufeZurueck()
...
# speichert die Startposition, um diese f�r Z-Bewegungen zu nutzen
...
def setzeStart(string):
    if(len(string) != 0):
        ...
        global startZy
        # Startposition
        # Wenn die Anfangskoordinaten teil eines gr��eren durch Leerzeichen getrennten Elements sind
        index = string.index(' ')
        pos = string[1: index]
        # print(pos)
        index = string.index(',')
...
# Bewegung zu zwei Koordinaten wird durchgef�hrt
...
# zur�ckkehren zur Startposition, nachdem
...
# der n�chste Character wird gesucht und sein Index zur�ckgegeben
...
# Hinzuf�gen der Punkte eines Moveto-Befehls zu den globalen Listen X,Y & Cmd
...
# Hinzuf�gen der Punkte eines Lineto-Befehls zu den globalen Listen X,Y & Cmd
...
# Hinzuf�gen der Punkte einer konvertierten Kurve zu den globalen Listen X,Y & Cmd
...
def macheCurve(curveCoord):
    ...
    while len(curveCoord) != 0:
        ...
        # X-Koordinate einf�gen
        index = curveCoord.index(' ')
        punkteX.append(float(curveCoord[0: index]))
        curveCoord = curveCoord[index+1: len(curveCoord)].lstrip()

        # Y-Koordinate einf�gen
        index = curveCoord.index(' ')
...
# �bergeben der linearen Liste von Punkten an der scaler
